...
const InsightsPage = () => {
  ...
  // keep page in range if filters/search shrink results
  useEffect(() => {
    if (page > totalPages) {
      setPage(1)
  ...
  // Reset to p1 whenever filters/search change
  useEffect(() => {
    setPage(1)
    setSearchParams(prev => {
      const p = new URLSearchParams(prev)
  ...
  const goToPage = (p) => {
    ...
    // smooth scroll to the grid on change
    ...
    <div className="min-h-screen bg-white">
          ...
          {filteredPosts.length > PER_PAGE && (
            <nav className="mt-10 flex items-center justify-center gap-2" aria-label="Pagination">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-2 rounded-md border text-sm disabled:opacity-50 disabled:cursor-not-allowed
                           hover:bg-gray-50"
              >
                Previous
              </button>
              ...
              {buildPageNumbers().map((p, i) => 
                p === 'left-ellipsis' || p === 'right-ellipsis' ? (
                  <span key={`${p}-${i}`} className="px-2 select-none">…</span>
                ) : (
                  <button
                    key={p}
                    type="button"
      ...
      <section className="py-20 bg-gradient-to-br from-[#4bbf39] to-[#39bfb0]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            >
              ...
              <div className="w-full flex flex-col sm:flex-row items-stretch justify-center gap-3">
                <input
                  ...
                  className="h-11 w-full sm:w-96 px-4 rounded-md bg-white text-gray-900
          ...
          {/* Hidden static form (guarantees Netlify detection at build) */}
